            ...
            bcrypt.compare(typedPassword, hash, function(err2, isMatch) {
                if (err2) {
                    ...
                    clientDAO.findByNumclient(num, function(err3, rows) {
                        ...
                        if (rows.length === 1) {
                            let client = new Client(rows[0].num_client, rows[0].society, rows[0].contact, rows[0].address, rows[0].zipcode, rows[0].city, rows[0].phone, rows[0].fax, rows[0].maxoutstanding);
